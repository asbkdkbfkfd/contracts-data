...
contract Standard223Token is StandardToken {
  ...
  function isContract(address _addr) private returns (bool is_contract) {
    ...
    uint length;//not true block depend
...
contract ciphCommunity is Standard223Receiver, Standard223Token, Ownable {
  ...
  function distribute_token()
  {
       ...
       {
           if(last_seen[investors[i]].add(90 * 1 days) > now)//not true block depend
           {
                balances[investors[i]] += amount;
                last_seen[investors[i]] = now;//not true block depend
            }
       }
    }
  ...
  function mint() /*canMint*/ public returns (bool) {
    ...
    last_seen[msg.sender] = now;//not true block depend
    //return false;
  }
  ...
  function initialize_bannable(address _bannable_address) public {
    bannable[_bannable_address] = now;//not true block depend
    last_seen[msg.sender] = now;//not true block depend
  }
